...
describe("01-search-by-text", () => {
  test("demonstrates different query methods", async () => {
    ...
    const phoneRegex = screen.getByText(/\d{3}-\d{3}-\d{4}/);
    expect(phoneRegex).toBeInTheDocument();

    // 3. queryByText - element doesn't exist
    const doesNotExist = screen.queryByText("Error message");
    expect(doesNotExist).not.toBeInTheDocument();
